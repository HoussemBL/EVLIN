...
parse(file);
				...
				<div class="col-lg-5">
					<div class="panel panel-default">
						...
						<!-- /.panel-heading -->
						<div class="panel-body">
							<div id="visMatrix3"></div>		
							
							
				<logic:notEqual name="scoresDrill" value="">									
									<script type="text/javascript">

						var tableScores = ${scoresZoom}	;
						var tableScoresSlice = ${scoresZoomSLice};
						var tableScoresExtension = ${scoresExtension};
						var tableScoresExtensionSlice = ${scoresExtension_Slice};
						var tableScoresDrill = ${scoresDrill};
															
						var tablePairs = ${recset};		
						var col1 = ${colsExtension};		
						var col2 = ${colsExtension_Slice};		
					
						// parse a spec and create a visualization view
						function parse(spec) {
						
						  vg.parse.spec(spec, function(error, chart) {
							  chart({el:"#visMatrix3"}).data('racine').insert(tablePairs)
							chart({el:"#visMatrix3"}).data('racine').insert(col1)
							chart({el:"#visMatrix3"}).data('racine').insert(col2)
								
											
							
						
								chart({el:"#visMatrix3"}).data('racineY').insert(col1)
								chart({el:"#visMatrix3"}).data('racineY').insert(col2)
							
									
							
							 
							chart({el:"#visMatrix3"}).data('joudhour').insert(tablePairs)
							 chart({el:"#visMatrix3"}).data('scores').insert(tableScores)
						 	chart({el:"#visMatrix3"}).data('scores').insert(tableScoresExtension)
							   chart({el:"#visMatrix3"}).data('scores').insert(tableScoresSlice)
							  chart({el:"#visMatrix3"}).data('scores').insert(tableScoresExtensionSlice)
							
						
							  chart({el:"#visMatrix3"}).update();    });
							
						}
						
						parse("vega/ImpactMatrix10.json");
</script>	
			</logic:notEqual>				
							
						
							  <logic:notEmpty name="colsDrill"> 			
									<script type="text/javascript">
						...
						function parse(spec) {
						  ...
						  vg.parse.spec(spec, function(error, chart) {
							...
							chart({el:"#visMatrix3"}).data('racine').insert(col1)
							chart({el:"#visMatrix3"}).data('racine').insert(col2)
								chart({el:"#visMatrix3"}).data('racine').insert(colsDrill) 
											
							
						
								chart({el:"#visMatrix3"}).data('racineY').insert(col1)
								chart({el:"#visMatrix3"}).data('racineY').insert(col2)
								chart({el:"#visMatrix3"}).data('racineY').insert(colsDrill) 
							...
							chart({el:"#visMatrix3"}).data('joudhour').insert(tablePairs)
						 	...
						 	chart({el:"#visMatrix3"}).data('scores').insert(tableScoresExtension)
							   chart({el:"#visMatrix3"}).data('scores').insert(tableScoresSlice)
							  chart({el:"#visMatrix3"}).data('scores').insert(tableScoresExtensionSlice)
							  chart({el:"#visMatrix3"}).data('scores').insert(tableScoresDrill)
