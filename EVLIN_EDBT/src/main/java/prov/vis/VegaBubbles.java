...
public  class VegaBubbles extends VegaVis {

	

	public VegaBubbles(QueryForm query) {
		super();
		this.query = query;
	}
	...
	//is used for recommendation zoom in
			...
			public VegaChart PrintVisrec_2D(String ColorCode) {
				...
				ScaleChart x = new ScaleChart("xscale", "ordinal", "width", true,true, xs,1,true);
				...
				List<Integer> range = new ArrayList<Integer>(Arrays.asList(0,2000));
				
				ScaleChart scSize = new ScaleChart("size","linear", ys, range);
				...
				ActChart acc3 = new ActChart("xscale", "tooltip.axeX2", 0);
				...
				Stream st3 = new Stream("dblclick", "open('result.jsp?cancel=" + labelX
						+ "&state='+datum.axeX2+'&chiffre='+datum.AggNumber,'_self')");
			...
			public VegaChart PrintVisrecZoomIn(String webField,String old,String type/*, List<DisplayForm> datasetToGiveColor*/){
				...
				List<DisplayForm> mapping = new ArrayList</*String*/DisplayForm>();
				...
				List<Integer> range = new ArrayList<Integer>(Arrays.asList(50,1000));
				
				ScaleChart scSize = new ScaleChart("size","linear", ys, range);
				
				
				
				List<ScaleChart> scls = new ArrayList<ScaleChart>(Arrays.asList(x, y,/* z,*/scSize,z_test));
				/********************* axis construction *************************/
				LabelAxeChart fill = new LabelAxeChart("black");
				LabelAxeChart angle = new LabelAxeChart(90);
				...
				/******text mark*******/		
						ActChart acc1 = new ActChart("center");
						ActChart acc2 = new ActChart("#333");
						ActChart acc3 = new ActChart("x", "tooltip.axeX2", 25);
				...
				Stream st3 = new Stream("symbol:dblclick",
						"open('result.jsp?cancel="+old2+"&state='+datum.axeX2+'&chiffre='+datum.sum_AggNumber+'&type="+type+"','_self')");
				List<Stream> sts = new ArrayList<Stream>(Arrays.asList(st1, st2, st3));
				Map<String, String> init = new HashMap<String, String>();
				Signal2 sig = new Signal2("tooltip", init, sts);
				List<SignalChart> sigs = new ArrayList<SignalChart>(Arrays.asList(sig));
		...
		//scales
				...
				ScaleChart z_size = new ScaleChart("size");
			...
			public void Complete_Rec_Viz_2D(VegaChart recommendation, List<DisplayForm> datasetToGiveColor,
				...
				//encoding channel related to size
				for (Entry<String, Integer> mark : mappingMarkID.entrySet()) {
					if (mark.getKey().equals("symbol")) {
				int chID = DistinctValues.connector.counter.newEncodingID();
				int chUsageID = DistinctValues.connector.counter.newchannelUsage_ID();
				ChannelTable idch = DistinctValues.connector.constructChannelRow(chID, "size", "AggNumber", session,
						scID12);
				ChannelUsageTable usageEncoRow = new ChannelUsageTable(chUsageID, chID, mark.getValue(),
						idch.getChanneltype());
				session.saveOrUpdate(idch);
				session.saveOrUpdate(usageEncoRow);}}
		...
		if(type.equals("Principal")){
			...
			public void Complete_Rec_Viz_Extension(VegaChart recommendation, String webField, String old, String type,
				...
				//Stream st3 = new Stream("rect:dblclick", "open('result.jsp?cancel=" + labelX+ "&state='+datum.axeX2+'&chiffre='+datum.sum_nummer+'&type=" + typeOp + "','_self')");
			//	j ai change ici, la valeur de cancel= n est pas connue encore webfield ou quoi?
